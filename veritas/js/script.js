(function(window,undefined){
  ...
  */
  Storenvy = {
    ...
    common: {
      init: function(){
        log('Storenvy.common.init()');

        if (Storenvy.isFirstRun) {
          Storenvy.common.firstRun();
          Storenvy.isFirstRun = false;
        };

      },
      firstRun: function(){
        log('Storenvy.common.firstRun()');

        // history.js
        Storenvy.historyConfig();
      }
    },

    home: {
      init: function(){
        log('Storenvy.home.init()');
    ...
    product: {
      init: function(){
        log('Storenvy.details.init()');

        $('.productdetails-gallery').cycle({
            fx:     'scrollHorz',
            timeout: 0,
            pager:  '.productdetails-gallery-nav'
        });
      }
    },

    historyConfig: function(){
      log('Storenvy.historyConfig()');

      //  make sure this function only runs once. bad things happen otherwise...
      // Storenvy.historyConfigured = true;
      ...
      // $(function(){
        var
          ...
          scrollOptions = {
            duration: Storenvy.settings.animSpeed,
            easing:'swing'
          };
        ...
        $(window).bind('statechange',function(){
          ...
          $content.animate({opacity:0},Storenvy.settings.animSpeed);
          ...
          $.ajax({
            ...
            success: function(data, textStatus, jqXHR){
              ...
              $content.html(contentHtml).ajaxify().animate({opacity:1},Storenvy.settings.animSpeed);
  ...
  }; // end Storenvy literal


  /**
      @namespace Utility object that powers the execution of specific functions in the Storenvy namespace based on data attributes on the <body> tag.
